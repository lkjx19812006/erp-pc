...
export const orderReceive = ({dispatch}, param) => { //订单收货流程
  ...
  if (param.image_s) {
    param.images += param.image_s + ','
  }
  if (param.image_t) {
    param.images += param.image_t
  }
  ;
  const body = {
    id: param.id
...
export const yankuanPayorder = ({dispatch}, param, undelinePay) => { //订单支付状态(已更改流程，此步骤接口未经过)
  ...
  if (undelinePay.image_s) {
    undelinePay.images += undelinePay.image_s + ','
  }
  if (undelinePay.image_t) {
    undelinePay.images += undelinePay.image_t
  }
  ;
  /*var ss= param.images;
...
export const customerTransferBlacklist = ({dispatch}, param) => { //客户转供应商,移出供应商,拉黑,移出黑名单
  ...
  Vue.http({
    ...
    if (param.link == '/customer/setSupplier') {
      dispatch(types.CUSTOMER_BATCH_SUPPLIER, param);
    }
    if (param.callback) {
      param.callback(res.json().msg);
    }
...
export const uploadFiles = ({dispatch}, param) => { //客户文件上传
  ...
  if (param.image_s && param.path) {
    param.path = param.path + ',' + param.image_s;
  } else if (param.image_s && !param.path) {
    param.path = param.image_s;
  }
  if (param.image_t && param.path) {
    param.path = param.path + ',' + param.image_t
...
export const baseAddData = ({dispatch}, param) => { //新增权限
  ...
  Vue.http({
    ...
    if (param.pid || res.json().code == 200) {
      ...
      if (res.json().result.id) {
        param.body.id = res.json().result.id;
      }
