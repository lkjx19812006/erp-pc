                ...
                <div class="client-section clearfix" @click.stop="">
                    ...
                    <ul class="clearfix" style="margin-top:20px;font-size: 13px; white-space: normal;">
                        <li class="col-md-6 col-xs-6">{{$t('static.breed')}}：
                            <label>{{initOrderDetail.goodsDesc}}</li>
                        <li class="col-md-6 col-xs-6">{{$t('static.client_name')}}：
                            <label>{{initOrderDetail.customerName}}</label>
                        </li>
                        <li class="col-md-6 col-xs-6">{{$t('static.client_phone')}}：
                            <label>{{initOrderDetail.customerPhone}}</li>
                        <li class="col-md-6 col-xs-6">{{$t('static.consignee_name')}}：
                            <label>{{initOrderDetail.consignee}}</li>
                        <li class="col-md-6 col-xs-6">{{$t('static.consignee_phone')}}：
                            <label>{{initOrderDetail.consigneePhone}}</li>
                        <li class="col-md-6 col-xs-6">{{$t('static.consignee_address')}}：
                            <label>{{initOrderDetail.consigneeAddr}}</li>
                        <li class="col-md-6 col-xs-6">{{$t('static.create_time')}}：
                            <label>{{initOrderDetail.ctime}}</li>
                    </ul>
                    <ul class="clearfix">
                        <li v-for="img in initOrderDetail.images" class="col-md-4">
                            <img :src="img.url" v-if="img.url | file" />
                            <a v-else href="{{img.url}}" download="" style="display: inline-block;width: 100px;margin-top: 10%"><img src="/static/images/{{$t('static.img_upload')}}.png" style="margin:auto" /></a>
                        </li>
                    </ul>
                </div>
            </section>
