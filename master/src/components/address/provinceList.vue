			...
			<div class="trans_service clearfix">
				...
				<div class="col-xs-8">
	                <div class="name_search clearfix">
	                    ...
	                    <input type="text" class="search_input" v-model="loadParam.name" placeholder="请输入省名称" @keyup.enter="">
	                </div>
	            </div>
	            <table class="table table-hover table_head table-striped " v-cloak>
	                ...
	                <tbody>
	                    <tr v-for="item in initProvince">
	                    	<td  @click.stop="">
	                           <label  class="checkbox_unselect" v-bind:class="{'checkbox_unselect':!item.checked,'checkbox_select':item.checked}"  @click="selectProvince($index,item.id,item.cname)" ></label>
	                        </td>
	                        <td>{{item.cname}}</td>
	                        <td></td>
	                    </tr>
...
export default{
	...
	methods:{
		selectProvince:function(index,id,cname){
			...
			this.$dispatch('province',this.param);
		},
	...
	created(){
		this.getProvinceList(this.param);
	}
}
</script>
<style scoped>
